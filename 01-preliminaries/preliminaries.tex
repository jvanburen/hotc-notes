...
\begin{mathpar}
  \inferrule[1a]{\alpha : \type \in \Gamma}{\cxtJ \alpha : \type}\and
  \inferrule[1b]{\cxtJ \tau_1 : \type \\
             \cxtJ \tau_1 : \type}
            {\cxtJ \tau_1 \to \tau_2 : \type}\and
  \inferrule[1c]{\Gamma, \alpha : \type \vdash \tau : \type}
            {\cxtJ \forall \alpha. \tau : \type}\and
  \inferrule[1d]{x : \tau \in \Gamma}{\cxtJ x : \tau}\and
  \inferrule{\Gamma, x : \tau_1 \vdash e : \tau_2 \\
             \cxtJ \tau : \type}
            {\cxtJ \lambda x : \tau_1. e : \tau_1 \to \tau_2}\and
  \inferrule[1e]{\cxtJ e_1 : \tau_1 \to \tau_2 \\
             \cxtJ e_2 : \tau_2}
            {\cxtJ e_1\ e_2 : \tau_2}\and
  \inferrule[1f]{\Gamma, \alpha : \type \vdash e : \tau}
            {\cxtJ \Lambda \alpha. e : \forall \alpha. \tau}\and
  \inferrule[1g]{\cxtJ e : \forall \alpha. \tau_2 \\
             \cxtJ \tau_1 : \type}
            {\cxtJ e[\tau_1] : [\tau_1/\alpha]\tau_2}\and
\end{mathpar}
...
\begin{mathpar}
  \inferrule[2a]{x : \tau \in \Gamma}{\cxtJ x : \tau}\and
  \inferrule[2b]{\Gamma, x : \tau_1 \vdash e : \tau_2 \\
             \cxtJ \tau : \type}
            {\cxtJ \lambda x : \tau_1. e : \tau_1 \to \tau_2}\and
  \inferrule[2c]{\cxtJ e_1 : \tau_1 \to \tau_2 \\
             \cxtJ e_2 : \tau_2}
            {\cxtJ e_1\ e_2 : \tau_2}\and
  \inferrule[2d]{\Gamma, \alpha : k \vdash e : \tau}
            {\cxtJ \Lambda \alpha : k. e : \forall \alpha : k. \tau}\and
  \inferrule[2e]{\cxtJ e : \forall \alpha : k. \tau_2 \\
             \cxtJ \tau_1 : k}
            {\cxtJ e[\tau_1] : [\tau_1/\alpha]\tau_2}\and
\end{mathpar}
...
\begin{mathpar}
  \inferrule[2f]{x : k \in \Gamma}{\cxtJ x : k}\and
  \inferrule[2g]{\Gamma, x : k_1 \vdash c : k_2}
            {\cxtJ \lambda x : k_1. c : k_1 \to k_2}\and
  \inferrule[2h]{\cxtJ c_1 : k_1 \to k_2 \\
             \cxtJ c_2 : k_2}
            {\cxtJ c_1\ c_2 : k_2}\and
  \inferrule[2i]{\Gamma, \alpha : k_1 \vdash c : \type}
            {\cxtJ \forall \alpha : k. c : \type}\and
  \inferrule[2j]{\cxtJ c_1 : \type \\
             \cxtJ c_2 : \type}
            {\cxtJ c_1 \to c_2 : \type}\and
\end{mathpar}
...
\begin{mathpar}
  \inferrule[2k]{\cxtJ c_1 \equiv c_2 : \type \\
             \cxtJ e : c_1}
            {\cxtJ e : c_2}
\end{mathpar}
...
relation: 
\begin{mathpar}
  \inferrule[3a]{ \quad }{\cxtJ c \equiv c : k}
  \and
  \inferrule[3b]{ \cxtJ c' \equiv c : k }{\cxtJ c \equiv c' : k}
  \and
  \inferrule[3c]{ \cxtJ c_1 \equiv c_2 : k \quad \cxtJ c_2 \equiv c_3 : k }{ \cxtJ c_1 \equiv c_3 : k }
\end{mathpar}
...
available to apply for each syntactic construct in the definition of types:
    ...
    \begin{mathpar}
        \inferrule[3d]{\cxtJ c_1 \equiv c_1' : k_1 \to k_2 \quad \cxtJ c_2 \equiv c_2' : k_2}{\cxtJ c_1 c_2 \equiv c_1' c_2' : k_2} \and
        \inferrule[3e]{\Gamma, \alpha : k_1 \vdash c \equiv c' : k_2}{\cxtJ (\lambda \alpha : k_1. \; c \equiv \lambda \alpha : k_1. \; c') : k_1 \to k_2 } \and
        \inferrule[3f]{\cxtJ \tau_1 \equiv \tau_2 : \type \\
                   \cxtJ \tau_1' \equiv \tau_2' : \type}{
                    \cxtJ (\tau_1 \to \tau_2) \equiv (\tau_1' \to \tau_2') : \type
                   } \and
        \inferrule[3g]{\Gamma, \alpha : k \vdash \tau \equiv \tau' : \type}{
            \cxtJ (\forall \alpha : k. \; \tau \equiv \forall \alpha : k. \; \tau') : \type
        } \and 
        \inferrule[3h]{\cxtJ c_2 : k \\
                   \Gamma, \alpha : k \vdash c_1 : k'}{
                    \cxtJ (\lambda \alpha : k. \; c_1) c_2 \equiv [c_2/\alpha]c_1 : k'
                   } \and
        \inferrule[3i]{\cxtJ c : k_1 \to k_2 \\
                   \cxtJ c' : k_1 \to k_2 \\
                   \Gamma, \alpha : k_1 \vdash c \, \alpha \equiv c' \, \alpha : k_2}{\cxtJ c \equiv c' : k_1 \to k_2}
    \end{mathpar}
...
These work as pairs of kinds. The kinding rules are as follows.
  \begin{mathpar}
    \inferrule[4a]{\cxtJ c_1 : k_1 \quad \cxtJ c_2 : k_2}{
      \cxtJ \langle c_1, c_2 \rangle : k_1 * k_2
    } \and
    \inferrule[4b]{\cxtJ c : k_1 * k_2 }{\cxtJ \pi_1 \ c : k_1} \and
    \inferrule[4c]{\cxtJ c : k_1 * k_2 }{\cxtJ \pi_2 \ c : k_2}
  \end{mathpar}
We also need to extend definitional equivalence to account for the new constructs.
  \begin{mathpar}
    \inferrule[4d]{\cxtJ c_1 \equiv c_1' : k_1 \quad \cxtJ c_2 \equiv c_2' : k_2}{\cxtJ \langle c_1, c_2 \rangle \equiv \langle c_1', c_2' \rangle : k_1 * k_2} \and
    \inferrule[4e]{\cxtJ c \equiv c' : k_1 * k_2}{\cxtJ \pi_1 \ c \equiv \pi_1 \ c' : k_1} \and
    \inferrule[4f]{\cxtJ c \equiv c' : k_1 * k_2}{\cxtJ \pi_2 \ c \equiv \pi_2 \ c' : k_2} \and
    \inferrule[4g]{\cxtJ c_1 : k_1 \quad \cxtJ c_2 : k_2}{\Gamma
      \vdash \pi_1 \angles{c_1, c_2} \equiv c_1} \and
    \inferrule[4h]{\cxtJ c_1 : k_1 \quad \cxtJ c_2 : k_2}{\cxtJ \pi_2 \langle c_1, c_2 \rangle \equiv c_2} \and
    \inferrule[4i]{\cxtJ \pi_1 \ c \equiv \pi_1 c' : k_1 \quad \cxtJ \pi_2 \ c \equiv \pi_2 \ c' : k_2}{\cxtJ c \equiv c' : k_1 * k_2}
  \end{mathpar}

\section{Type Inference}
We present a few kinds of algorithms: annotated with inputs$^+$ and outputs$^-$:
\[\begin{array}{rcll}
  \cxt^+\vdash e^+ &\impl& \tau^- &\text{type synthesis}\\
  \cxt^+\vdash e^+ &\impl &\tau^+& \text{type checking}\\
  \cxt^+\vdash c^+ &\impl& k^-& \text{kind synthesis}\\
  \cxt^+\vdash c^+ &\limpl &k^+& \text{kind checking}\\
  \cxt^+\vdash c^+& \lrimpl &c'^+ : k^+& \text{algorithmic equivalence}\\
  \cxt^+\vdash c^+& \lrImpl &c'^+ : k^+& \text{algorithmic path equivalence}\\
  c^+ &\Downarrow &c'^- & \text{weak-head normalization}\\
  c^+ &\leadsto &c'^- & \text{weak-head reduction}\\
\end{array}
\]

\begin{mathpar}
  \inferrule[5a]{\cxtJ \lam{x:\tau}e\implies \tau\to\tau'}{\cxtJ \tau\impliedby T \\ \cxt, x:\tau\vdash e \implies \tau'}

  \inferrule[5b]{\cxtJ e_1 e_2\impl \tau'}{\cxtJ e_1\impl\tau_1}
  % CR jvanbure: get notes from Corwin :C
\end{mathpar}

% CR jvanbure: get notes from Alice :(

\begin{mathpar}
  \inferrule{\cxtJ\alpha\impl k}{\G(\alpha) = k}

  \inferrule{\cxtJ\lam{\alpha:k}c\impl k\to k'}{\G,\alpha :k\vdash c\impl k'}
  
  \inferrule{\cxtJ c \limpl k}{\cxtJ c\impl k' \\ k=k'}
...
\begin{mathpar}
  \inferrule{c\Downarrow c''}{c~>c' \\ c'\Downarrow c''}

  \inferrule{c\Downarrow c}{``c{\,\not{\!\!\ape}} \text{anything}''}
